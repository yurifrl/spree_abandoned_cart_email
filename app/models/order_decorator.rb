Spree::Order.class_eval do
  ...
  def self.eligible_abandoned_email_orders
    where("state != ? AND (payment_state IS NULL OR payment_state != ?) AND email is NOT NULL AND abandoned_email_sent_at IS NULL AND created_at < ?", "complete", "paid", (Time.zone.now - Spree::AbandonedCartEmailConfig::Config.email_timeframe))
  end
